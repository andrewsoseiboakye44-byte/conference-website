...
async function fetchActiveGatewayFromSupabase() {
  try {
    ...
    console.warn('Could not query Supabase for gateway settings in serverless function:', err);
  }
  return null;
}
...
async function logSmsAttempt({ phone, message, provider, status, errorMessage, campaignType }) {
  try {
    ...
    console.warn('Could not write sms_log in serverless function:', err);
  }
}
...
async function dispatchToProvider({ phone, message, gateway }) {
  ...
  if (provider === 'arkesel') {
    ...
    // Fallback: Try Arkesel v1 Query API
    try {
...
async function processSendSms(body) {
  const {
    phone,
    message,
    gateway,
  ...
  try {
    ...
    return {
      status: 500,
      data: { error: errorText, provider: gw.provider },
    };
  }
}
...
module.exports = async function handler(req, res) {
  ...
  const result = await processSendSms(body);
...
async function netlifyHandler(event) {
  ...
  try {
    ...
    return { statusCode: 400, headers: CORS_HEADERS, body: JSON.stringify({ error: 'Invalid JSON payload in request body.' }) };
  }
  const result = await processSendSms(body);
  return {
